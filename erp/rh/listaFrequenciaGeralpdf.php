...
function calcularHorasTrabalhadas($entrada, $saida_intervalo, $retorno_intervalo, $saida_final) {
    if (!$entrada || !$saida_final) return 0;
...
function calcularAdicionalNoturno($entrada, $saida_intervalo, $retorno_intervalo, $saida_final) {
    ...
    $saida_min = timeToMinutes($saida_final);
    
    // Período noturno (22h às 5h = 1320min às 300min)
    $inicioNoturno = 22 * 60;    // 22:00 = 1320min
    $fimNoturno = 5 * 60;        // 05:00 = 300min
    
    // Se saída for no dia seguinte (após meia-noite)
    if ($saida_min < $entrada_min) $saida_min += 1440; // Adiciona 24h em minutos
    
    // Verifica tempo antes do intervalo
    if ($saida_intervalo) {
        $saida_intervalo_min = timeToMinutes($saida_intervalo);
        $totalMinutosNoturnos += calcularMinutosNoturnos($entrada_min, $saida_intervalo_min, $inicioNoturno, $fimNoturno);
    }
    
    // Verifica tempo após o intervalo
    if ($retorno_intervalo && $saida_final) {
        $retorno_intervalo_min = timeToMinutes($retorno_intervalo);
        $totalMinutosNoturnos += calcularMinutosNoturnos($retorno_intervalo_min, $saida_min, $inicioNoturno, $fimNoturno);
    } elseif ($saida_final) {
        $totalMinutosNoturnos += calcularMinutosNoturnos($entrada_min, $saida_min, $inicioNoturno, $fimNoturno);
    }
    
    return $totalMinutosNoturnos;
}

function calcularMinutosNoturnos($inicio, $fim, $inicioNoturno, $fimNoturno) {
    $minutosNoturnos = 0;
    
    // Se o período atravessa a meia-noite
    if ($inicio < $fimNoturno && $fim > $inicioNoturno) {
        if ($inicio < $fimNoturno) {
            $minutosNoturnos += min($fimNoturno, $fim) - $inicio;
        }
        if ($fim > $inicioNoturno) {
            $minutosNoturnos += $fim - max($inicioNoturno, $inicio);
        }
    } 
    // Período noturno normal (22h-5h)
    elseif ($inicio >= $inicioNoturno && $fim <= $fimNoturno + 1440) {
        $minutosNoturnos += $fim - $inicio;
    }
    // Parte do período é noturno
    elseif ($inicio < $inicioNoturno && $fim > $inicioNoturno) {
        $minutosNoturnos += $fim - $inicioNoturno;
    } 
    elseif ($inicio < $fimNoturno && $fim > $fimNoturno) {
        $minutosNoturnos += $fimNoturno - $inicio;
    }
...
function calcularCargaHorariaDia($funcionario) {
    ...
    $saida = $funcionario['saida_final'];
    $saida_intervalo = $funcionario['saida_intervalo'];
    $retorno_intervalo = $funcionario['retorno_intervalo'];

    if (!$entrada || !$saida) return '--:--';

    $minutos = calcularHorasTrabalhadas($entrada, $saida_intervalo, $retorno_intervalo, $saida);
...
try {
    ...
    if (empty($funcionarios)) {
        echo "<script>alert('Nenhum funcionário encontrado para o período selecionado'); history.back();</script>";
        exit;
    }

    // Exiba apenas UM funcionário por vez (por exemplo, o primeiro da lista)
    if (!empty($funcionarios)) {
                                            ...
                                            <table class="report-table">
                                                ...
                                                <tbody>
                                                    <?php foreach ($pontos as $ponto):
                                                        ...
                                                        // Verifica se a carga horária trabalhada é maior ou igual à esperada
                                                        $horasTrabalhadas = calcularHorasTrabalhadas(
                                                            $ponto['entrada'],
                                                            $ponto['saida_intervalo'],
                                                            $ponto['retorno_intervalo'],
                                                        ...
                                                        $cargaHorariaEsperada = timeToMinutes($funcionario['saida_final']) - timeToMinutes($funcionario['entrada']);
                                                        
                                                        if ($horasTrabalhadas < $cargaHorariaEsperada) {
                                                            $normal = false;
                                                        }
                                                        ...
                                                        if ($minutosNoturnos > 0) {
                                                            $ocorrencias[] = 'Adicional Noturno: ' . minutesToHM($minutosNoturnos);
                                                        }
                                                    ?>
                                                    <tr>
                                                        <td><?= formatarData($ponto['data']) ?></td>
                                                        ...
                                                        <td><?= calcularCargaHorariaDia($funcionario) ?></td>
                                                        <td><?= $normal ? 'Normal' : implode(', ', $ocorrencias) ?></td>
                                                    </tr>
                                                    <?php endforeach; ?>
                                                </tbody>
                                            </table>
