...
try {
  $stmt = $pdo->prepare("SELECT * FROM setores WHERE id_selecionado = :id_selecionado");
  $stmt->bindParam(':id_selecionado', $idSelecionado, PDO::PARAM_STR);
  $stmt->execute();
      ...
      <div class="layout-page">
          ...
          <div class="navbar-nav-right d-flex align-items-center" id="navbar-collapse">
            ...
            <div class="navbar-nav align-items-center">
              <div class="nav-item d-flex align-items-center">
                ...
                <input type="text" id="searchSetor" class="form-control border-0 shadow-none" placeholder="Search..."
                  aria-label="Search..." />
              </div>
            </div>
            <!-- /Search -->
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
          ...
          <h4 class="fw-bold mb-0"><span class="text-muted fw-light"><a
                href="#">Setores</a>/</span>Adicionados</h4>
          <h5 class="fw-bold mt-3 mb-3 custor-font"><span class="text-muted fw-light">Visualize os Setores da sua
              Empresa</span></h5>

          <!-- Card -->
          <div class="card mt-3">
            <h5 class="card-header">Lista de Setores</h5>
            ...
            <div class="table-responsive text-nowrap">
              <table class="table table-hover">
                ...
                <tbody id="tabelaSetores" class="table-border-bottom-0">
                    ...
                    <tr>
                      ...
                      <td>
                        ...
                        <button class="btn btn-link text-primary p-0" title="Editar"
                          onclick="window.location.href='editarSetor.php?id=<?= $setor['id'] ?>&idSelecionado=<?= urlencode($idSelecionado) ?>'">
                        ...
                        <button class="btn btn-link text-danger p-0" title="Excluir" data-bs-toggle="modal"
                          data-bs-target="#modalExcluir_<?= $setor['id'] ?>">
                          <i class="tf-icons bx bx-trash"></i>
                        </button>

                        <!-- Modal -->
                        <div class="modal fade" id="modalExcluir_<?= $setor['id'] ?>" tabindex="-1"
                          aria-labelledby="modalExcluirLabel_<?= $setor['id'] ?>" aria-hidden="true">
                          <div class="modal-dialog">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h5 class="modal-title" id="modalExcluirLabel_<?= $setor['id'] ?>">Excluir Setor</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                  aria-label="Close"></button>
                              </div>
                              <div class="modal-body">
                                <p>Tem certeza de que deseja excluir este setor?</p>
                                <a href="../../assets/php/rh/excluirSetor.php?id=<?= $setor['id'] ?>&idSelecionado=<?= urlencode($idSelecionado) ?>"
            ...
            <div class="d-flex justify-content-start align-items-center gap-2 m-3">
              <div>
                <button id="prevPageSetor" class="btn btn-sm btn-outline-secondary">Anterior</button>
                <div id="paginacaoSetor" class="btn-group"></div>
                <button id="nextPageSetor" class="btn btn-sm btn-outline-secondary">Próximo</button>
              </div>
          ...
          <!-- ✅ Script -->
          <script>
            const searchSetor = document.getElementById('searchSetor');
            const linhasSetor = Array.from(document.querySelectorAll('#tabelaSetores tr'));
            const rowsPerPageSetor = 10;
            let currentPageSetor = 1;

            function renderTableSetor() {
              const filtro = searchSetor.value.toLowerCase();
              const filtradas = linhasSetor.filter(linha =>
                Array.from(linha.querySelectorAll('td')).some(td =>
                  td.textContent.toLowerCase().includes(filtro)
                )
              );

              const totalPages = Math.ceil(filtradas.length / rowsPerPageSetor);
              const inicio = (currentPageSetor - 1) * rowsPerPageSetor;
              const fim = inicio + rowsPerPageSetor;

              linhasSetor.forEach(l => l.style.display = 'none');
              filtradas.slice(inicio, fim).forEach(l => l.style.display = '');

              const paginacao = document.getElementById('paginacaoSetor');
              paginacao.innerHTML = '';
              ...
              document.getElementById('prevPageSetor').disabled = currentPageSetor === 1;
              document.getElementById('nextPageSetor').disabled = currentPageSetor === totalPages || totalPages === 0;
            }

            searchSetor.addEventListener('input', () => {
              currentPageSetor = 1;
              renderTableSetor();
            });

            document.getElementById('prevPageSetor').addEventListener('click', () => {
              if (currentPageSetor > 1) {
                currentPageSetor--;
                renderTableSetor();
              }
            });

            document.getElementById('nextPageSetor').addEventListener('click', () => {
              const filtro = searchSetor.value.toLowerCase();
              const totalFiltradas = linhasSetor.filter(linha =>
                Array.from(linha.querySelectorAll('td')).some(td =>
                  td.textContent.toLowerCase().includes(filtro)
                )
              ).length;
              const totalPages = Math.ceil(totalFiltradas / rowsPerPageSetor);
              if (currentPageSetor < totalPages) {
                currentPageSetor++;
                renderTableSetor();
              }
            });
          ...
          <div id="addsetor" class="mt-3 add-category justify-content-center d-flex text-center align-items-center"
            onclick="window.location.href='adicionarSetor.php?id=<?= urlencode($idSelecionado) ?>';"
