...
if (!str_starts_with($idSelecionado, 'principal_') && !str_starts_with($idSelecionado, 'unidade_')) {
...
try {
  // Empresa principal
  $stmtPrincipal = $pdo->query("SELECT nome_empresa FROM sobre_empresa WHERE id_selecionado = 'principal_1' LIMIT 1");
  ...
  while ($unidade = $stmtUnidades->fetch(PDO::FETCH_ASSOC)) {
    $empresas[] = [
      ...
      'nome' => $unidade['nome'] . " - ({$unidade['tipo']})"
    ];
  }
} catch (PDOException $e) {
  echo "<script>alert('Erro ao carregar empresas!'); history.back();</script>";
  exit;
}
?>
      ...
      <div class="authentication-inner">
        ...
        <div class="card">
          <div class="card-body">
            ...
            <form id="formAuthentication" class="mb-3" action="../erp/login/processarCadastro.php" method="POST">
