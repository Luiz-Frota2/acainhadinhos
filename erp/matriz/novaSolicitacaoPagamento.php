...
if (str_starts_with($idSelecionado, 'principal_')) {
    // página pode ser acessada pela matriz, mas a ideia é a FILIAL solicitar
...
/** Define a matriz de destino. Ajuste se você tiver relacionamento em tabela própria */
function resolve_id_matriz(string $idSelecionado): string {
    // regra simples: sempre envia para principal_1
    return 'principal_1';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (!isset($_POST['csrf']) || $_POST['csrf'] !== $_SESSION['csrf_pagamento']) {
        ...
        // Normaliza/valida vencimento (YYYY-MM-DD)
        ...
        if (isset($_FILES['arquivo']) && is_uploaded_file($_FILES['arquivo']['tmp_name'])) {
            ...
            // URL pública relativa ao projeto
            $comprovante_url = "../../assets/uploads/pagamentos/" . $newName;
        }
        ...
        // Limpa CSRF para evitar re-post
        unset($_SESSION['csrf_pagamento']);

        // Redireciona com mensagem
        $_SESSION['flash_msg'] = 'Solicitação registrada com sucesso!';
        header("Location: ./solicitarPagamentoConta.php?id=" . urlencode($idSelecionado));
        exit;
...
<head>
    ...
    <style>
        ...
        .form-section-title { font-weight: 700; font-size: 1rem; color: #374151; }
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
                <?php if ($erro): ?>
                    <div class="alert alert-danger alert-dismissible" role="alert">
                        <span class="fw-semibold">Erro:</span> <?= htmlspecialchars($erro, ENT_QUOTES) ?>
                ...
                <div class="card">
                    <div class="card-body">
                        <form method="post" enctype="multipart/form-data" id="formNovaSolic" novalidate>
...
(function () {
    ...
    // máscara suave para valor (permitindo digitar livre e normalizando visual)
    const valorInput = document.querySelector('input[name="valor"]');
    valorInput.addEventListener('blur', () => {
        let v = valorInput.value.trim();
        if (!v) return;
        // normaliza para formato BR ao perder foco
        v = v.replace(/\./g,'').replace(',', '.');
        const n = parseFloat(v);
        if (!isNaN(n)) {
            valorInput.value = n.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2});
        }
    });
})();
</script>
