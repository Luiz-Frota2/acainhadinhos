...
if (!isset($pdo) || !($pdo instanceof PDO)) {
  http_response_code(500); echo "Erro: conexão indisponível."; exit;
}
...
/* ============================================================
   ESCOPO: SOMENTE FRANQUIAS
   - JOIN com 'unidades u' pelo número de s.id_solicitante (ex.: "unidade_1" => 1)
   - u.tipo = 'Franquia'
   - Franquia logada vê apenas suas solicitações
   - Demais perfis enxergam por matriz atual
   ============================================================ */
$scopeWhere  = [];
$scopeParams = [];
...
/* ===== AJAX: Itens (HTML tabela compacta) ===== */
if (isset($_GET['ajax']) && $_GET['ajax'] === 'itens') {
  header('Content-Type: text/html; charset=UTF-8');
  $sid = (int)($_GET['sid'] ?? 0);
  if ($sid <= 0) { http_response_code(400); echo '<div class="text-danger p-2">Solicitação inválida.</div>'; exit; }
  ...
  if ($tipoSession === 'franquia') {
    $ok = $pdo->prepare("
      ...
      WHERE s.id = :id AND u.tipo = 'Franquia'
    ...
    $ok = $pdo->prepare("
      ...
      FROM solicitacoes_b2b s
      JOIN unidades u ON u.id = CAST(SUBSTRING_INDEX(s.id_solicitante, '_', -1) AS UNSIGNED)
      WHERE s.id = :id AND s.id_matriz = :mat AND u.tipo = 'Franquia'
    ");
    $ok->execute([':id' => $sid, ':mat' => $idSelecionado]);
  }
  if (!$ok->fetchColumn()) { http_response_code(403); echo '<div class="text-danger p-2">Acesso negado.</div>'; exit; }
  ...
  if (!$itens) { echo '<div class="text-muted p-2">Nenhum item nesta solicitação.</div>'; exit; } ?>
  <div class="table-responsive">
    <table class="table table-sm table-hover">
      <thead>
        <tr>
          <th>#</th><th>SKU</th><th>Produto</th>
          <th class="text-end">Qtde</th><th>Unid.</th>
  ...
  if ($term !== '' && mb_strlen($term) >= 2) {
    if ($tipoSession === 'franquia') {
      ...
      $s1 = $pdo->prepare("
        ...
        FROM solicitacoes_b2b s
        JOIN unidades u ON u.id = CAST(SUBSTRING_INDEX(s.id_solicitante, '_', -1) AS UNSIGNED)
        WHERE u.tipo = 'Franquia' AND s.id_matriz = :matriz
          AND s.id_solicitante LIKE :q
        ORDER BY s.id_solicitante LIMIT 10
      ");
  ...
  if ($tipoSession !== 'principal') {
    ...
    try {
      $st = $pdo->prepare("
        ...
        FROM solicitacoes_b2b s
        JOIN unidades u ON u.id = CAST(SUBSTRING_INDEX(s.id_solicitante, '_', -1) AS UNSIGNED)
        WHERE s.id = :id AND s.id_matriz = :matriz AND u.tipo = 'Franquia'
      ");
      ...
      if (!$row) {
        ...
        if ($id_matriz_post !== $row['id_matriz'] || ($id_solicitante_post && $id_solicitante_post !== $row['id_solicitante'])) {
          ...
          switch ($acao) {
            case 'aprovar':
              if ($statusAtual === 'pendente') { $novoStatus = 'aprovada'; $setTime['aprovada_em'] = date('Y-m-d H:i:s'); }
              break;
            case 'reprovar':
            ...
            case 'cancelar':
              ...
              break;
            case 'enviar':
              if ($statusAtual === 'aprovada') { $novoStatus = 'em_transito'; $setTime['enviada_em'] = date('Y-m-d H:i:s'); }
              break;
            case 'entregar':
              if ($statusAtual === 'em_transito') { $novoStatus = 'entregue'; $setTime['entregue_em'] = date('Y-m-d H:i:s'); }
              break;
          }
          if (!$novoStatus) {
            ...
            foreach ($setTime as $col => $val) { $sql .= ", {$col}=:{$col}"; $params[":{$col}"] = $val; }
...
/* ===== Filtros + Paginação ===== */
$perPage = max(5, min(100, (int)($_GET['pp'] ?? 20))); // permite ?pp=40 por ex.
...
if ($de !== '' && preg_match('/^\d{4}-\d{2}-\d{2}$/', $de)) {
  $where[] = "DATE(s.created_at) >= :de"; $params[':de'] = $de;
}
if ($ate !== '' && preg_match('/^\d{4}-\d{2}-\d{2}$/', $ate)) {
  $where[] = "DATE(s.created_at) <= :ate"; $params[':ate'] = $ate;
}
if ($q !== '') {
  $where[] = "(
...
/* ===== Helper URL paginação ===== */
$qs = $_GET; $qs['id'] = $idSelecionado; $range = 2;
$makeUrl = function ($p) use ($qs) { $qs['p'] = $p; return '?' . http_build_query($qs); };
...
<head>
  ...
  <style>
    ...
    .small-muted{ font-size:.8rem; color:#8b98a8 }
    .autocomplete{ position:relative }
    .autocomplete-list{ position:absolute; top:100%; left:0; right:0; max-height:260px; overflow:auto; background:#fff; border:1px solid #e6e9ef; border-radius:.5rem; box-shadow:0 10px 24px rgba(24,28,50,.12); z-index:2060 }
...
<body>
    ...
    <div class="layout-container">
      ...
      <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
        ...
        <ul class="menu-inner py-1">
          ...
          <li class="menu-item "><a href="javascript:void(0);" class="menu-link menu-toggle"><i class="menu-icon tf-icons bx bx-building"></i><div>Franquias</div></a>
            <ul class="menu-sub"><li class="menu-item"><a href="./franquiaAdicionada.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Adicionadas</div></a></li></ul>
          </li>
          <li class="menu-item active open">
            ...
            <ul class="menu-sub">
              <li class="menu-item"><a href="./contasFranquia.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Pagamentos Solic.</div></a></li>
              <li class="menu-item active"><a href="#" class="menu-link"><div>Produtos Solicitados</div></a></li>
          ...
          <li class="menu-item"><a href="javascript:void(0);" class="menu-link menu-toggle"><i class="menu-icon tf-icons bx bx-bar-chart-alt-2"></i><div>Relatórios</div></a>
            <ul class="menu-sub">
              <li class="menu-item"><a href="./VendasFranquias.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Vendas por Franquias</div></a></li>
              <li class="menu-item"><a href="./MaisVendidos.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Mais Vendidos</div></a></li>
              <li class="menu-item"><a href="./FinanceiroFranquia.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Financeiro</div></a></li>
            </ul>
          </li>
            ...
            <ul class="navbar-nav flex-row align-items-center ms-auto">
              <li class="nav-item navbar-dropdown dropdown-user dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end">
                  <li><a class="dropdown-item" href="#"><div class="d-flex">
                    <div class="flex-shrink-0 me-3"><div class="avatar avatar-online"><img src="<?= e($logoEmpresa) ?>" class="w-px-40 h-auto rounded-circle" /></div></div>
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
              ...
              <div class="row g-3 align-items-end">
                ...
                <div class="col-12 col-md flex-grow-1 filter-col">
                  ...
                  <div class="autocomplete">
                    <input type="text" class="form-control form-control-sm" id="qInput" name="q" placeholder="Solicitante (ex.: franquia_3), SKU ou Produto…" value="<?= e($q) ?>" autocomplete="off">
                    <div class="autocomplete-list d-none" id="qList"></div>
                  </div>
                </div>
                <div class="col-12 col-md-auto filter-col">
                  <label class="form-label mb-1">Por página</label>
                  <select class="form-select form-select-sm" name="pp">
                    <?php foreach ([10,20,30,40,50,100] as $opt): ?>
          ...
          <div class="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-2">
            ...
            <ul class="pagination mb-0">
              ...
              <?php
                $start = max(1, $page - $range); $end = min($totalPages, $page + $range);
          ...
          <div class="card table-zone">
              ...
              <div class="table-responsive text-nowrap">
                <table class="table table-hover align-middle">
                  ...
                  <tbody class="table-border-bottom-0">
                    <?php if (!$rows): ?>
                      <tr><td colspan="10" class="text-center text-muted py-4">Nenhuma solicitação encontrada.</td></tr>
            ...
            <div class="card-footer d-flex justify-content-between align-items-center flex-wrap gap-2">
              ...
              <span class="small text-muted">Mostrando <?= (int)($totalRows ? ($offset + 1) : 0) ?>–<?= (int)min($totalRows, $offset + $perPage) ?> de <?= (int)$totalRows ?></span>
            </div>
            <?php endif; ?>
          </div>
  ...
  <script>
    ...
    modalDetalhes.addEventListener('show.bs.modal', function(event) {
      ...
      if (!sid) { body.innerHTML = '<div class="text-danger">Solicitação inválida.</div>'; return; }
    ...
    function optionsForStatus(st) {
      if (st === 'pendente') return [{v:'aprovar',t:'Aprovar'},{v:'reprovar',t:'Reprovar'},{v:'cancelar',t:'Cancelar'}];
      if (st === 'aprovada') return [{v:'enviar',t:'Marcar Em Trânsito'},{v:'cancelar',t:'Cancelar'}];
      if (st === 'em_transito') return [{v:'entregar',t:'Marcar Entregue'}];
      return [];
    }
    ...
    document.getElementById('formStatus').addEventListener('submit', function(e) {
      if (!msSelect.value) { e.preventDefault(); return; }
    ...
    (function() {
      ...
      function pick(i){ if(i<0||i>=items.length)return; qInput.value = items[i].value; closeList(); form.submit(); }
      ...
      qInput.addEventListener('input', function(){
        ...
        const url = new URL(window.location.href);
        url.searchParams.set('ajax','autocomplete'); url.searchParams.set('q', v);
      ...
      document.addEventListener('click', function(e){ if(!list.contains(e.target) && e.target!==qInput) closeList(); });

      function highlight(){ [...list.querySelectorAll('.autocomplete-item')].forEach((el,idx)=>el.classList.toggle('active', idx===activeIndex)); }
