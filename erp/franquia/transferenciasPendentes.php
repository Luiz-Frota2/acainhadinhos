      ...
      <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
        ...
        <ul class="menu-inner py-1">
          ...
          <li class="menu-item">
            ...
            <ul class="menu-sub">
              <li class="menu-item"><a href="./franquiaAdicionada.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Adicionadas</div></a></li>
            </ul>
          </li>
          ...
          <li class="menu-item active open">
            ...
            <ul class="menu-sub">
              ...
              <li class="menu-item"><a href="./politicasEnvio.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Política de Envio</div></a></li>
          ...
          <li class="menu-item">
            ...
            <ul class="menu-sub">
              <li class="menu-item"><a href="./VendasFiliais.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Vendas por Franquias</div></a></li>
              <li class="menu-item"><a href="./MaisVendidos.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Mais Vendidos</div></a></li>
              <li class="menu-item"><a href="./vendasPeriodo.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Vendas por Período</div></a></li>
      ...
      <div class="layout-page">
        ...
        <nav class="layout-navbar container-xxl navbar navbar-expand-xl navbar-detached align-items-center bg-navbar-theme" id="layout-navbar">
          ...
          <div class="navbar-nav-right d-flex align-items-center" id="navbar-collapse">
            <form class="d-none d-md-block ms-auto" method="get">
              <input type="hidden" name="id" value="<?= e($idSelecionado) ?>">
              <div class="nav-item d-flex align-items-center">
                <i class="bx bx-search fs-4 lh-0"></i>
                <input name="q" value="<?= e($busca) ?>" type="text" class="form-control border-0 shadow-none" placeholder="Buscar código, filial..." aria-label="Buscar..." />
              </div>
            </form>
          ...
          <div class="card">
            ...
            <div class="table-responsive text-nowrap">
              <table class="table table-hover">
                ...
                <?php if (!$linhas): ?>
                  <tr><td colspan="8" class="text-center text-muted">Nenhuma transferência encontrada.</td></tr>
                <?php else: ?>
                  <?php foreach ($linhas as $r): 
                      $stKey = $r['status'];
                      $st    = $mapStatus[$stKey] ?? ['label'=>$stKey,'class'=>'bg-label-secondary'];
                  ?>
                  <tr>
                    <td><strong><?= e($r['codigo'] ?? ('TR-'.$r['id'])) ?></strong></td>
                    <td><?= e($r['filial_nome']) ?></td>
                    <td><?= (int)$r['itens_total'] ?></td>
                    <td><?= (int)$r['qtd_total'] ?></td>
                    <td><?= $r['criado_em'] ? date('d/m/Y H:i', strtotime($r['criado_em'])) : '-' ?></td>
                    <td><?= $r['enviado_em'] ? date('d/m/Y H:i', strtotime($r['enviado_em'])) : '-' ?></td>
                    <td><span class="badge <?= e($st['class']) ?> status-badge"><?= e($st['label']) ?></span></td>
                    <td class="text-end actions">
                      <button 
                        ...
                        data-id="<?= (int)$r['id'] ?>"
                        data-codigo="<?= e($r['codigo'] ?? ('TR-'.$r['id'])) ?>"
                        data-filial="<?= e($r['filial_nome']) ?>"
                        data-status="<?= e($st['label']) ?>"
                        >
                        Detalhes
                      </button>

                      <?php if (in_array($r['status'], ['aguardando','enviado'], true)): ?>
                        <form class="d-inline" method="post">
                          <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
                          <input type="hidden" name="transferencia_id" value="<?= (int)$r['id'] ?>">
                          <input type="hidden" name="acao" value="confirmar_envio">
                          <button class="btn btn-sm btn-warning" onclick="return confirm('Confirmar envio desta transferência?')">Confirmar envio</button>
                        </form>
                      <?php endif; ?>

                      <?php if (in_array($r['status'], ['enviado','em_transito'], true)): ?>
                        <form class="d-inline" method="post">
                          <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
                          <input type="hidden" name="transferencia_id" value="<?= (int)$r['id'] ?>">
                          <input type="hidden" name="acao" value="confirmar_recebimento">
                          <button class="btn btn-sm btn-success" onclick="return confirm('Marcar como recebido?')">Marcar recebido</button>
                        </form>
                      <?php endif; ?>

                      <?php if (in_array($r['status'], ['aguardando','enviado','em_transito'], true)): ?>
                        <form class="d-inline" method="post">
                          <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
                          <input type="hidden" name="transferencia_id" value="<?= (int)$r['id'] ?>">
