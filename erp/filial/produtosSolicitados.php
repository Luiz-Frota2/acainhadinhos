...
if (!function_exists('descobrirColunasItens')) {
  function descobrirColunasItens(PDO $pdo): array
  {
    ...
    $colCod = null;
    foreach (['produto_codigo', 'codigo_produto', 'sku', 'codigo', 'qr_code', 'cod_produto', 'id_produto'] as $c) if (in_array($c, $cols, true)) {
      $colCod = $c;
      break;
    }
    $colQtd = null;
    foreach (['quantidade', 'qtd', 'qtde', 'quantidade_solicitada', 'qtd_solicitada'] as $c) if (in_array($c, $cols, true)) {
      $colQtd = $c;
      break;
    }
    $colNome = null;
    foreach (['nome_produto', 'descricao_produto', 'produto_nome'] as $c) if (in_array($c, $cols, true)) {
      $colNome = $c;
      break;
    }
...
if (isset($_GET['acao']) && $_GET['acao'] === 'detalhes') {
  ...
  if (!$empresaIdMatrizAjax) {
    http_response_code(401);
    echo "<div class='text-danger'>Sessão expirada.</div>";
    exit;
  }
  ...
  <script>
    (function() {
      ...
      function setRowStatus(pedidoId, novoStatus) {
        ...
        switch (novoStatus) {
          case 'aprovada':
            badge = '<span class="badge bg-label-primary status-badge">Aprovada</span>';
            break;
          case 'reprovada':
            badge = '<span class="badge bg-label-danger status-badge">Reprovada</span>';
            break;
          case 'pendente':
            badge = '<span class="badge bg-label-warning status-badge">Pendente</span>';
            break;
          default:
            badge = '<span class="badge bg-label-secondary status-badge">' + (novoStatus || '—') + '</span>';
        }
