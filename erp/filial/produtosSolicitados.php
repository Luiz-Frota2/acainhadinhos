      ...
      <ul class="menu-inner py-1">
        ...
        <li class="menu-item open active">
          ...
          <ul class="menu-sub active">
            ...
            <li class="menu-item active"><a href="./produtosSolicitados.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Produtos Solicitados</div></a></li>
              ...
              <ul class="dropdown-menu dropdown-menu-end">
                <li>
                  <a class="dropdown-item" href="#">
                    <div class="d-flex">
                      <div class="flex-shrink-0 me-3"><div class="avatar avatar-online"><img src="<?= h($logoEmpresa) ?>" class="w-px-40 h-auto rounded-circle" /></div></div>
...
(function () {
  ...
  function setRowStatus(pedidoId, novoStatus) {
    ...
    // Se virou aprovada/reprovada, remove botões Aprovar/Reprovar
  ...
  document.querySelectorAll('.btn-detalhes').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!resp.ok) { alvo.innerHTML = `<div class="text-danger">Falha ao carregar detalhes (HTTP ${resp.status}).</div>`; return; }
  ...
  async function postAjax(fd) {
    let resp;
    try {
      resp = await fetch(SELF, { method: 'POST', body: fd, credentials: 'same-origin', headers: { 'X-Requested-With': 'XMLHttpRequest' } });
    } catch (netErr) {
  ...
  document.getElementById('btnConfirmAprovar')?.addEventListener('click', async (ev) => {
    ...
    if (j && j.ok) {
      ...
      // recarrega para refletir filtro, se desejar pode comentar
