...
function pad6(string $n): string { $n = onlyDigits($n); return str_pad($n === '' ? '0' : $n, 6, '0', STR_PAD_LEFT); }
...
/* ======================== LER CONFIG (arquivo + BD) ======================== */
$cfgArq = [
    'tpAmb' => null, 'cnpj' => null, 'razao_social' => null,
    'CSC' => null, 'CSCid' => null, 'pfx_path' => null, 'pfx_password' => null,
];
...
if ($tpAmb === 1) { $ambienteStatus = 'Produção';     $ambienteClass = 'bg-label-primary'; }
elseif ($tpAmb===2){ $ambienteStatus = 'Homologação';  $ambienteClass = 'bg-label-info'; }
...
if ($pfxPath) {
    ...
    if (is_file($pfxAbs)) {
        if ($pfxPass !== null && $pfxPass !== '') {
            ...
            if ($ok) { $certificadoStatus = 'Válido'; $certificadoClass = 'bg-label-success'; }
            else     { $certificadoStatus = 'Arquivo encontrado (senha inválida?)'; $certificadoClass = 'bg-label-danger'; }
        } else {
            $certificadoStatus = 'Arquivo encontrado (sem senha)'; $certificadoClass = 'bg-label-warning';
        }
    } else {
        $certificadoStatus = 'Arquivo não encontrado'; $certificadoClass = 'bg-label-danger';
    }
} else {
    $certificadoStatus = 'Não informado'; $certificadoClass = 'bg-label-secondary';
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        pre.code { background:#0f172a; color:#e2e8f0; border-radius:8px; padding:12px; overflow:auto; font-size:12px; }
        ...
        <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
            ...
            <ul class="menu-inner py-1">
                <li class="menu-item"><a href="index.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link">
                    <i class="menu-icon tf-icons bx bx-home-circle"></i><div>Dashboard</div></a></li>

                <li class="menu-header small text-uppercase"><span class="menu-header-text">Frente de Caixa</span></li>
                <li class="menu-item">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">
                        <i class="menu-icon tf-icons bx bx-barcode-reader"></i><div>Operações de Caixa</div>
                    </a>
                    <ul class="menu-sub">
                        <li class="menu-item"><a href="./abrirCaixa.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Abrir Caixa</div></a></li>
                        <li class="menu-item"><a href="./fecharCaixa.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Fechar Caixa</div></a></li>
                        <li class="menu-item"><a href="./sangria.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Sangria</div></a></li>
                        <li class="menu-item"><a href="./suprimento.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Suprimento</div></a></li>
                    </ul>
                </li>
                <li class="menu-item"><a href="./vendaRapida.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link">
                    <i class="menu-icon tf-icons bx bx-cart-alt"></i><div>Venda Rápida</div></a></li>
                <li class="menu-item"><a href="./cancelarVenda.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link">
                    <i class="menu-icon tf-icons bx bx-x-circle"></i><div>Cancelar Venda</div></a></li>

                <li class="menu-item open active">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">
                        <i class="menu-icon tf-icons bx bx-bar-chart-alt"></i><div>Relatórios</div>
                    </a>
                    <ul class="menu-sub">
                        <li class="menu-item"><a href="./relatorioVendas.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Resumo de Vendas</div></a></li>
                        <li class="menu-item"><a href="./relatorioVendas.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link"><div>Detalhes da Venda</div></a></li>
                    </ul>
                </li>

                <li class="menu-header small text-uppercase"><span class="menu-header-text">Diversos</span></li>
                <li class="menu-item"><a href="../sistemadeponto/index.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link">
                    <i class="menu-icon tf-icons bx bx-group"></i><div>Sistema de Ponto</div></a></li>
                <li class="menu-item"><a href="../Delivery/index.php?id=<?= urlencode($idSelecionado); ?>" class="menu-link">
                    <i class="menu-icon tf-icons bx bx-cart"></i><div>Delivery</div></a></li>
                <li class="menu-item"><a href="https://wa.me/92991515710" target="_blank" class="menu-link">
                    <i class="menu-icon tf-icons bx bx-support"></i><div>Suporte</div></a></li>
            </ul>
        </aside>
        <!-- /Sidebar -->
        ...
        <div class="layout-page">
                    ...
                    <ul class="navbar-nav flex-row align-items-center ms-auto">
                        <li class="nav-item navbar-dropdown dropdown-user dropdown">
                            <a class="nav-link dropdown-toggle hide-arrow" href="javascript:void(0);" data-bs-toggle="dropdown">
                                <div class="avatar avatar-online"><img src="<?= htmlspecialchars($logoEmpresa) ?>" alt class="w-px-40 h-auto rounded-circle" /></div>
                            </a>
                            <ul class="dropdown-menu dropdown-menu-end">
                                <li><a class="dropdown-item" href="#">
                                    <div class="d-flex">
                                        <div class="flex-shrink-0 me-3"><div class="avatar avatar-online"><img src="<?= htmlspecialchars($logoEmpresa) ?>" class="w-px-40 h-auto rounded-circle" /></div></div>
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div>
                        <h4 class="fw-bold mb-0">Documentação — NFC-e</h4>
                        <div class="text-muted">Empresa: <b class="text-primary"><?= htmlspecialchars($idSelecionado) ?></b></div>
                    </div>
                    <div class="d-flex gap-2">
                        <a class="btn btn-outline-secondary btn-sm" href="visualizarNFCe.php?id=<?= urlencode($idSelecionado) ?>" target="_blank">
                            <i class="fa-solid fa-eye me-1"></i> Visualizar Modelo
                        </a>
                        <a class="btn btn-primary btn-sm" href="adicionarNFCe.php?id=<?= urlencode($idSelecionado) ?>">
                            <i class="fa-solid fa-gear me-1"></i> Configurar NFC-e
                        </a>
                    </div>
                </div>
                ...
                <div class="row mt-4">
                    ...
                    <div class="col-lg-3">
                        <div class="card h-100">
                            <div class="card-header"><h5 class="card-title m-0">Sumário</h5></div>
                    ...
                    <div class="col-lg-9">
                        ...
                        <div id="sec-arquivos" class="card mb-4">
                            <div class="card-header"><h5 class="card-title m-0">2) Arquivos e Pastas</h5></div>
                            <div class="card-body">
                                <ul>
                                    ...
                                    <li><code>../../nfce/</code> — pasta onde ficam os XMLs autorizados (ex.: <code>procNFCe_&lt;chave&gt;.xml</code>).</li>
...
chown www-data:www-data ../../assets/img/certificado/seu_certificado.pfx
                        ...
                        <div id="sec-config" class="card mb-4">
                            <div class="card-header"><h5 class="card-title m-0">3) Configuração</h5></div>
                            <div class="card-body">
...
define('PFX_PASSWORD', 'sua_senha_forte');</code></pre>
                        ...
                        <div id="sec-visualizacao" class="card mb-4">
                            <div class="card-header"><h5 class="card-title m-0">4) Visualização do DANFE</h5></div>
                            <div class="card-body">
...
# parâmetros opcionais: fantasia, razao, cnpj, serie, nnf, valor, tpag, qrcode
                        ...
                        <div id="sec-botao" class="card mb-4">
                            <div class="card-header"><h5 class="card-title m-0">5) Botões e Rotas</h5></div>
                            <div class="card-body">
...
&lt;a class="btn btn-primary btn-sm"
   href="danfe_nfce.php?id=&lt;?= urlencode($idSelecionado) ?&gt;&amp;venda_id=&lt;?= (int)$vendaId ?&gt;&amp;chave=&lt;?= urlencode($chave) ?&gt;"
   target="_blank"&gt;
   Ver DANFE (XML)
&lt;/a&gt;</code></pre>
                        ...
                        <div id="sec-erros" class="card mb-4">
                            <div class="card-header"><h5 class="card-title m-0">6) Erros Comuns &amp; Soluções</h5></div>
                            <div class="card-body">
                                <ul class="mb-0">
                        ...
                        <div id="sec-checklist" class="card mb-4">
                            <div class="card-header"><h5 class="card-title m-0">7) Checklist Rápido</h5></div>
                            <div class="card-body">
                                <ol class="mb-2">
