...
if (!empty($_SESSION['carrinho'])) {
    foreach ($_SESSION['carrinho'] as $index => $item) {
        echo "<div class='card mb-2 pr-0'>
                <div class='container-detalhes'>
                    <div class='detalhes-produto'>
                        <div class='infos-produto'>
                            <p class='name'><b>". $item['quant']  ."x ". $item['nome'] . "<br></b></p>
                            <p class='price'><b>R$" . $item['preco'] . "</b></p>
                        </div>
                        <div class='infos-produto'>
                            <p class='name-opcional mb-0'>1x Granola</p>
               ...
               ";
        // Botão para editar e remover
        echo "<form action='remove_from_cart.php' method='post' style='margin-top:5px;'>
                <input type='hidden' name='index' value='{$index}'>
                  <div class='detalhes-produto-edit'>
                  <button type='submit' class='btn btn-link text-danger p-0' title='Excluir'><i class='tf-icons bx bx-trash'></i></button>
                   </div>
                </form>";
        echo "</div>
         </div>
