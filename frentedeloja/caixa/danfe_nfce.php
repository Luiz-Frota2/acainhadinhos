...
// Uso: danfe_nfce.php?id=<empresa_id>&venda_id=123&chave=NNNN... (44 dígitos)
...
// AJUSTE DE CAMINHO DA CONEXÃO (este arquivo está em frentedeloja/caixa/)
        ...
        if ($chaveReq || $vendaId > 0) {
            $sql = "SELECT xml_nfeproc
               ...
               WHERE empresa_id = :emp
                 " . ($chaveReq ? "AND chave = :chave" : "AND venda_id = :venda") . "
               ORDER BY id DESC
               LIMIT 1";
            $st = $pdo->prepare($sql);
            $bind = [':emp' => $empresaId];
...
if (!$file && !$xmlRaw) {
    ...
    die('Arquivo da NFC-e não encontrado em ../../nfce/ nem nos fallbacks' . $hint . '.');
}
