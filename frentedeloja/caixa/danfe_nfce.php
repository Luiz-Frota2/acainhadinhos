...
//
// Uso típico: danfe_nfce.php?id=<empresa_id>&venda_id=123&chave=NNNN... (44 dígitos)
...
// Atualiza a venda com a chave, se soubermos tudo
try {
...
function fmtChave($ch){ $ch = preg_replace('/\D+/', '', $ch); return trim(implode(' ', str_split($ch, 4))); }
function mapTPag($t){
...
} elseif (!empty($chaveReq)) {
  $file = $base . 'procNFCe_' . preg_replace('/\D+/', '', (string)$chaveReq) . '.xml';
} else {
  die('Informe ?chave=... (44) ou ?arq=procNFCe_....xml');
}
if (!is_file($file)) {
  die('Arquivo não encontrado: ' . htmlspecialchars($file));
}

// ====================== Carrega XML com performance =================
...
<div class="wrapper" role="document" aria-label="DANFE NFC-e">
  ...
  <table class="tbl small" aria-label="Totais">
    <tbody>
      ...
      <tr><td class="left"><b>VALOR TOTAL R$</b></td><td class="right"><?= $vNF ?></td></tr>
  ...
  <?php if (!empty($protInfo)): ?><div class="small"><?= htmlspecialchars($protInfo) ?></div><?php endif; ?>
</div>

<!-- Barra de ações -->
<div class="actions" aria-label="Ações">
  <?php if ($empresaId): ?>
    <a class="btn btn-secondary" href="../frentedeloja/caixa/vendaRapida.php?id=<?= urlencode($empresaId) ?>">← Voltar para PDV</a>
  <?php endif; ?>
  <button id="btn-print" class="btn btn-primary" type="button">Imprimir</button>
</div>

<!-- QRCode JS (pequeno e rápido no CDN) -->
