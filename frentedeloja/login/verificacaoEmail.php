...
try {
    $stmt = $pdo->prepare("SELECT id, usuario, codigo_verificacao, codigo_verificacao_expires_at FROM funcionarios_acesso WHERE email = :email AND empresa_id = :empresa_id AND tipo = :tipo LIMIT 1");
        ...
        if (empty($codigoAtual) || time() > $codigoExpiracao) {
            ...
            if (mail($email, $assunto, $mensagem, $headers)) {
                ...
                echo "<script>
                    ...
                    window.location.href = '../../../../frentedeloja/verificarCodigo.php?id={$idUrl}&email={$emailUrl}';
                </script>";
            } else {
