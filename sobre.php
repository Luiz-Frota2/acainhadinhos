...
try {
    ...
    if ($info) {
        ...
        if (!empty($info['imagem'])) {
            $imagemEmpresa = "./assets/img/uploads/" . $info['imagem'];
        }
    }
...
try {
    ...
    if ($end) {
        $enderecoCompleto = $end['endereco'] . ", " . $end['numero'] . 
                            " - " . $end['bairro'] . ", " .
                            $end['cidade'] . " - " . $end['uf'];
    }
} catch (PDOException $e) {}
...
try {
    ...
    if ($pg) {
        $pagamento["dinheiro"]      = $pg["dinheiro"] == 1;
        $pagamento["pix"]           = $pg["pix"] == 1;
        $pagamento["cartaoDebito"]  = $pg["cartaoDebito"] == 1;
        $pagamento["cartaoCredito"] = $pg["cartaoCredito"] == 1;
    }
} catch (PDOException $e) {}
    ...
    <div class="container-group mb-5">
        ...
        <?php if (array_filter($pagamento)): ?>
            <?php if ($pagamento["dinheiro"]): ?>
                <div class="card mt-2"><p class="normal-text mb-0"><b>Dinheiro</b></p></div>
            <?php endif; ?>

            <?php if ($pagamento["pix"]): ?>
                <div class="card mt-2"><p class="normal-text mb-0"><b>Pix</b></p></div>
            <?php endif; ?>

            <?php if ($pagamento["cartaoDebito"]): ?>
                <div class="card mt-2"><p class="normal-text mb-0"><b>Cartão Débito</b></p></div>
            <?php endif; ?>

            <?php if ($pagamento["cartaoCredito"]): ?>
                <div class="card mt-2"><p class="normal-text mb-0"><b>Cartão Crédito</b></p></div>
            <?php endif; ?>
        <?php else: ?>
            <div class="card mt-2"><p class="normal-text mb-0">Nenhuma forma de pagamento cadastrada</p></div>
        <?php endif; ?>
    </div>

</section>

<a href="./index.php?empresa=<?= urlencode($empresaID) ?>" class="btn btn-yellow btn-full voltar">
