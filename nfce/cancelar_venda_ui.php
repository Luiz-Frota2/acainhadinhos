...
// Captura empresa_id de forma flexível
$__empresa_id = $_GET['id'] ?? $_GET['empresa_id'] ?? $_REQUEST['id'] ?? $_REQUEST['empresa_id'] ?? '';
// Chave da NFC-e (nota a ser cancelada)
$__chave = isset($chave) ? preg_replace('/\D+/', '', (string)$chave) : '';

?>
<style>
/* ===== Modal básica (sem libs) ===== */
#cv-overlay { display:none; position:fixed; inset:0; background:rgba(0,0,0,.45); z-index:9998; }
.cv-modal { display:none; position:fixed; z-index:9999; inset:0; place-items:center; }
.cv-card { width: min(560px, 92vw); background:#fff; border-radius:14px; box-shadow:0 10px 30px rgba(0,0,0,.25); overflow:hidden; font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,"Helvetica Neue",Arial; }
.cv-head { padding:14px 18px; font-size:18px; font-weight:700; background:#0d6efd; color:#fff; }
.cv-body { padding:18px; }
.cv-row { margin-bottom:12px; }
.cv-row label{ display:block; font-weight:600; margin-bottom:6px; }
.cv-row input[type=text],
.cv-row textarea{ width:100%; padding:10px 12px; border:1px solid #cfd6df; border-radius:10px; outline:0; font-size:15px; }
.cv-row input[readonly]{ background:#f6f7fb; }
.cv-foot { padding:14px 18px; background:#f9fafc; display:flex; gap:10px; justify-content:flex-end; border-top:1px solid #eef2f6; }
.cv-btn { appearance:none; border:0; border-radius:10px; padding:10px 14px; font-weight:700; cursor:pointer; }
.cv-btn.primary{ background:#0d6efd; color:#fff; }
.cv-btn.danger{ background:#dc3545; color:#fff; }
.cv-btn.secondary{ background:#e9ecef; color:#111; }
.cv-choices { display:grid; grid-template-columns:1fr; gap:12px; }
.cv-choice { border:1px solid #cfd6df; border-radius:12px; padding:14px; cursor:pointer; transition:transform .08s ease; }
.cv-choice:hover{ transform:translateY(-1px); background:#f8f9ff; }
.cv-choice .title { font-weight:800; }
.cv-choice .desc { opacity:.85; font-size:14px; margin-top:4px; }
@media (min-width:680px){
  .cv-choices { grid-template-columns:1fr 1fr 1fr; }
}
.cv-note { font-size:12.5px; opacity:.85; margin-top:8px; }
</style>
...
<div id="cv-modal-escolha" class="cv-modal" role="dialog" aria-modal="true" aria-labelledby="cv-escolha-title">
  <div class="cv-card">
    ...
    <div class="cv-body">
      ...
      <div class="cv-note">Dica: use <b>Substituição (110112)</b> quando emitiu outra NFC-e para a mesma operação (ex.: contingência).</div>
    </div>
    <div class="cv-foot">
...
<!-- Modal 2: Por CHAVE (confirmação 4 últimos dígitos) -->
  ...
  <div class="cv-card">
    ...
    <form id="cv-form-chave" action="cancelar_venda_processa.php" method="post" class="no-print">
      <div class="cv-body">
        ...
        <input type="hidden" name="empresa_id" id="cv-empresa-id" value="<?= htmlspecialchars($__empresa_id) ?>">
      </div>
      <div class="cv-foot">
  ...
  <div class="cv-card">
    ...
    <form id="cv-form-motivo" action="cancelar_venda_processa.php" method="post" class="no-print">
      <div class="cv-body">
        ...
        <input type="hidden" name="chave" id="cv-chave2" value="<?= htmlspecialchars($__chave) ?>">
        <input type="hidden" name="empresa_id" id="cv-empresa-id2" value="<?= htmlspecialchars($__empresa_id) ?>">
      </div>
      <div class="cv-foot">
        <button type="button" class="cv-btn secondary" data-cv-back>Voltar</button>
...
<!-- Modal 4: Cancelamento por Substituição (110112) -->
  ...
  <div class="cv-card">
    ...
    <form id="cv-form-subst" action="cancelar_venda_processa.php" method="post" class="no-print">
      <div class="cv-body">
        ...
        <input type="hidden" name="empresa_id" id="cv-empresa-id3" value="<?= htmlspecialchars($__empresa_id) ?>">
      </div>
      <div class="cv-foot">
        <button type="button" class="cv-btn secondary" data-cv-back>Voltar</button>
...
(function(){
  ...
  function openModal(el){ overlay.style.display='block'; el.style.display='grid'; }
  function closeAll(){ overlay.style.display='none'; mEscolha.style.display='none'; mChave.style.display='none'; mMotivo.style.display='none'; mSubst.style.display='none'; }

  if (btnAbrir) { btnAbrir.addEventListener('click', function(){ openModal(mEscolha); }); }

  document.querySelectorAll('[data-cv-close]').forEach(function(el){ el.addEventListener('click', closeAll); });
  document.querySelectorAll('[data-cv-back]').forEach(function(el){
    el.addEventListener('click', function(){ mChave.style.display='none'; mMotivo.style.display='none'; mSubst.style.display='none'; openModal(mEscolha); });
  });
  ...
  document.querySelectorAll('.cv-choice').forEach(function(card){
    card.addEventListener('click', function(){
      ...
      if (modelo === 'por_chave'){ openModal(mChave); setTimeout(function(){ last4Input && last4Input.focus(); }, 120); }
      else if (modelo === 'por_motivo'){ openModal(mMotivo); setTimeout(function(){ var m = document.getElementById('cv-motivo'); m && m.focus(); }, 120); }
      else { openModal(mSubst); setTimeout(function(){ chaveSubst && chaveSubst.focus(); }, 120); }
    });
  });
  ...
  if (formChave) {
    formChave.addEventListener('submit', function(ev){
      ...
      if (chv.length !== 44) { ev.preventDefault(); alert('Chave inválida (44 dígitos).'); return false; }
      if (last4.length !== 4) { ev.preventDefault(); alert('Digite os 4 últimos dígitos da chave.'); return false; }
      if (chv.slice(-4) !== last4) { ev.preventDefault(); alert('Os 4 últimos dígitos não conferem.'); return false; }
    });
  }
  ...
  if (formMotivo) {
    formMotivo.addEventListener('submit', function(ev){
      ...
      if (!motivo) { ev.preventDefault(); alert('Informe o motivo do cancelamento.'); return false; }
    });
  }
  ...
  if (formSubst) {
    formSubst.addEventListener('submit', function(ev){
      ...
      if (chv.length !== 44) { ev.preventDefault(); alert('Chave (original) inválida (44 dígitos).'); return false; }
      if (sub.length !== 44) { ev.preventDefault(); alert('Chave substituta inválida (44 dígitos).'); return false; }
      if (sub === chv) { ev.preventDefault(); alert('A chave substituta deve ser diferente da chave a cancelar.'); return false; }
    });
  }
